  ...
  return (
    <AppShell title="Categorias" subtitle="Base de classificação financeira para sustentar DFC, leitura operacional e automações futuras.">
      ...
      {sessionLoading || loading ? (
        <div className="oc-shell-card" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 22, padding: 24, color: '#475467', boxShadow: '0 16px 40px rgba(15,23,42,0.05)' }}>Carregando categorias...</div>
      ) : !companyId ? (
        <div className="oc-shell-card" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 22, padding: 24 }}>Faça login para carregar as categorias.</div>
      ) : (
        <div className="oc-grid-main" style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 24, alignItems: 'start' }}>
          <div className="oc-shell-card" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 24, overflow: 'hidden', boxShadow: '0 16px 40px rgba(15,23,42,0.05)' }}>
            <div style={{ padding: 20, borderBottom: '1px solid #eaecf0', fontWeight: 800, color: '#101828' }}>Estrutura de categorias</div>
            <div className="oc-table-wrap">
              <table className="oc-table-mobile" style={{ width: '100%', borderCollapse: 'collapse' }}>
